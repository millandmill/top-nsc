<?php 
    ...
    $text = "<font color='red'>No Receipt data, Please add Billingnote first.</font>";
    if(isset($invoice)){
        $resultData = $invoice;
    }
    if(isset($search)){
    ...
    $secoundID = "receipt_id";
    $dataShow = array(
        'date' => array('วันที่', 'receipt_date', '13%', 'center', 'center'),
        'PO#' => array('RC&nbsp;#', 'receipt_text', '10%', 'center', 'center'),
        'detail' => array('รายละเอียด', 'customer_name', '32%', 'left', 'center'),
        'price' => array('มูลค่า', 'product_grand_total', '12%', 'center', 'center'),
        'status' => array('สถานะ', 'receipt_status', '15%', 'center', 'center'),
            ...
            <div class="content">
                <form action="receipt_display" method="POST">
                    <div class="header-content">
                        <div class="font24"><i class="glyphicon glyphicon-bitcoin"></i>ใบเสร็จรับเงิน
                                            ...
                                            <tr>
                                                <?php foreach ($dataShow as $key => $value) { ?>
                                                    <?php switch($key){
                                                        ...
                                                        case 'option': ?>
                                                            <td style="text-align:center;">
                                                                <button name="edit" class="btn btn-grey btn-o" onclick="location.href='<?php echo base_url(); ?>receipt/receipt_add?r_id=<?php echo $secID; ?>&q_id=<?php echo $rowID; ?>';" title="แก้ไข"><i class="fa fa-edit "></i></button>
                                                                <button name="detail" class="btn btn-grey btn-o" onclick="javascript:window.open('<?php echo base_url(); ?>generate_pdf/generate_receipt?receipt_paper=<?php echo $rows['receipt_paper']; ?>&type=receipt');" title="ดูรายละเอียด"><i class="fa fa-search-plus"></i></button>
                                                                <button name="copy_print" class="btn btn-grey btn-o" onclick="javascript:window.open('<?php echo base_url(); ?>generate_pdf/generate_receipt?receipt_paper=<?php echo $rows['receipt_paper']; ?>&type=receipt_copy');" title="ดูสำเนาใบเสร็จรับเงิน"><i class="fa fa-copy" ></i></button>
                                                                <!--<button name="dele" class="btn btn-grey btn-o" onclick="location.href='<?php echo base_url(); ?>receipt/change_status?q_id=<?php echo $rowID; ?>&r_id=<?php echo $secID; ?>';" title="ยกเลิก"><i class="fa fa-close"></i></button>-->
                                                            </td>
                                                            <?php break;
                                                        case 'status': ?>
                                                        ...
                                                        case 'PO#' ?>
                                                            <td style="text-align:<?php echo $dataShow[$key][3]; ?>">
                                                                <?php echo $rows[$dataShow[$key][1]].$rows['receipt_number']; ?>
                                                            </td>
                                                            <?php break;
                                                        default: ?>
                                                            <td style="text-align:<?php echo $dataShow[$key][3]; ?>" >
